...
bool SM_ExecuteConfig(IPlugin *pl, AutoConfig *cfg, bool can_create)
{
	...
	{
		/* read the existing cvars in the file so the new/missing cvars get added */
		fp = fopen(file, "rt");
		goto cfg_read;
	}
...
cfg_read:
	if (!fp)
	{
		logger->LogError("Failed to read config for %s, make sure the directory has read permission.", pl->GetFilename());
		return can_create;
	}
